...
```{r output-setting, message=FALSE}
...
1. Add an overall column to each treatment arm.

2. Label the overall column as `Total`.

3. In the header, place the number of subjects on a new line in each treatment arm. 

4. Add a table caption that states `2014 Disorders Adverse Events` (displays at the top of the table).
...
t2_shell %>%  
 as_hux_xlsx(file = "exercises/output/summary_shell.xlsx" )
```
